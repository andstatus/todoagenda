...
class SamsungTasksProvider(type: EventProviderType, context: Context, widgetId: Int) :
    ...
    private val whereClause: String
        get() {
            ...
            whereBuilder.append(SamsungTasksContract.Tasks.COLUMN_DELETED).append(EQUALS)
                .append("0")
            if (filterMode == FilterMode.NORMAL_FILTER) {
                whereBuilder.append(AND).append(SamsungTasksContract.Tasks.COLUMN_COMPLETE)
                    .append(
                        EQUALS
                    ).append("0")
            }
    ...
    private fun newTask(cursor: Cursor): TaskEvent {
        ...
        task.color = getColor(
            cursor, cursor.getColumnIndex(SamsungTasksContract.Tasks.COLUMN_COLOR),
            cursor.getInt(cursor.getColumnIndex(SamsungTasksContract.Tasks.COLUMN_LIST_ID))
        )
        return task
    }
    ...
    private fun getColor(cursor: Cursor, colorIdx: Int, accountId: Int): Int {
        return if (!cursor.isNull(colorIdx)) {
